...
from tables_mstmerchandise import mst_merchandise
from tables_tblmessage import tbl_message
from tables_tblmoney import tbl_money
from tables_tblstock import tbl_stock
...
# 商品データ書き出し
productidlist = session.query(mst_merchandise.id).all()
productnamelist = session.query(mst_merchandise.name).all()
productpricelist = session.query(mst_merchandise.price).all()
print(productidlist)

product_stock = session.query(tbl_stock.stock).filter_by(id=str(productidlist[0][1:-2]))
print(product_stock)
...
for i in range(0,4):
    ...
    if int(product_stock) > 0:
        print(str(productnamelist[i])[2:-3],end="")
        print("：",end="")
        print(str(productpricelist[i])[1:-2],end="")
        print("円")
